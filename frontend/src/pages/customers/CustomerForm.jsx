...
import { clearSelectedCustomer, createCustomer, getCustomer, updateCustomer } from '../../store/slices/customerSlice'
...
const CustomerForm = () => {
  ...
  const { selectedCustomer, isLoading, isError, message } = useSelector((state) => state.customers)
  ...
  const onSubmit = async (data) => {
    try {
      if (isEdit) {
        await dispatch(updateCustomer({ id, customerData: data })).unwrap()
        toast.success('Customer updated')
      } else {
        const companyId = user?.role === 'super_admin' ? currentCompany?._id : undefined
        await dispatch(createCustomer({ customerData: data, companyId })).unwrap()
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold text-gray-800 mb-6">{isEdit ? 'Edit Customer' : 'New Customer'}</h1>
